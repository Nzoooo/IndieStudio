...
template <typename T> T *ecs::World::get()
{
    if (has<T>() == false)
...
template <typename T, typename... U> void ecs::World::add(U &&...args)
{
    if (has<T>() == true)
...
template <typename T> bool ecs::World::has() const
{
    unsigned int id = ecs::TemplateSystem<T>::getId();
    return id < _systems.size() && _systems[id];
}

template <typename T> void ecs::World::remove()
{
    if (has<T>() == false)
        std::cout << "System not found" << std::endl;
